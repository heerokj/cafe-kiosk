...
public class MenuBoardPage4 extends JFrame implements MouseListener, ActionListener {
	...
	JPanel jpMenu, jpCoffee, jpNonCoffee, jpSide; //jpMenu은 뭘까

	DefaultTableModel modelList;

	JTable jtbList; //주문리스트
	...
	public MenuBoardPage4(String id) { 
		...
		Image changeLogo = logo.getImage().getScaledInstance(100, 100, Image.SCALE_SMOOTH); //이미지 크기변환
		logo.setImage(changeLogo);  //logo에 이미지 크기 변환한걸 set하기(?)
